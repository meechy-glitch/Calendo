...
function getPostPlatformsForDate(posts: MobilePost[], date: Date): Platform[] {
  return [...new Set(posts.filter((post) => isSameDay(post.date, date)).map((p) => p.platform))]
}
...
function MobileDayCell({
  date, posts, isCurrentMonth, isToday, isSelected, onSelect,
}: {
  date: Date; posts: MobilePost[]; isCurrentMonth: boolean; isToday: boolean
  isSelected: boolean; onSelect: (date: Date) => void
}) {
  ...
  return (
    <button
      ...
      className={cn("flex flex-col items-center gap-1 py-2 transition-colors", !isCurrentMonth && "opacity-30")}
    >
      <span
        className={cn(
      ...
      <div className="flex h-2 items-center gap-0.5">
        {visiblePlatforms.map((platform, index) => (
          <div key={index} className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: PLATFORM_COLORS[platform] }} />
        ))}
        {hasMore && <span className="text-[8px] font-medium text-[#888888]">+</span>}
      </div>
    </button>
  )
}
...
function PostListItem({
  post, onClick, onMarkPublished,
}: {
  post: MobilePost; onClick: (post: MobilePost) => void; onMarkPublished: (post: MobilePost) => void
}) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <div className="w-1 self-stretch rounded-full" style={{ backgroundColor: color, minHeight: "40px" }} />
        <div className="flex flex-1 flex-col gap-1">
          <span
            className="text-sm font-medium"
            style={{ color: "#F5F5F5", opacity: isPublished ? 0.6 : 1, textDecoration: isPublished ? "line-through" : "none" }}
          >
            {post.title}
          </span>
          {post.scheduledTime && (
            <span className="text-xs" style={{ color: "#888888" }}>{formatTime12h(post.scheduledTime)}</span>
          )}
          <span className="text-xs capitalize" style={{ color: "#888888" }}>{post.platform}</span>
        </div>
...
export function CalendarMobile({
  posts, currentMonth, onDateClick, onPostClick, onMonthChange, onMarkPublished,
}: CalendarMobileProps) {
    ...
    <div className="relative flex h-full flex-col bg-[#0F0F0F]">
      <div className="flex flex-col gap-2 border-b border-[#2A2A2A] p-4">
        <div className="flex items-center justify-between">
          <Button variant="ghost" size="icon" onClick={handlePrevMonth} className="h-8 w-8 text-[#F5F5F5] hover:bg-[#2A2A2A]">
          ...
          <Button variant="ghost" size="icon" onClick={handleNextMonth} className="h-8 w-8 text-[#F5F5F5] hover:bg-[#2A2A2A]">
        ...
        <div className="grid grid-cols-7">
          {DAYS_OF_WEEK.map((day, index) => (
            <div key={index} className="py-1 text-center text-xs font-medium text-[#888888]">{day}</div>
          ))}
        </div>
      ...
      <div className="flex flex-1 flex-col gap-3 overflow-auto p-4">
        <h3 className="text-sm font-semibold text-[#F5F5F5]">{selectedDateFormatted}</h3>
        ...
        {selectedDatePosts.length === 0 ? (
          ...
          <div className="flex flex-col gap-2" style={{ overflowY: "auto", maxHeight: "calc(100vh - 480px)", paddingBottom: "140px" }}>
            {selectedDatePosts.map((post) => (
              <PostListItem key={post.id} post={post} onClick={onPostClick} onMarkPublished={onMarkPublished} />
            ))}
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="fixed bottom-20 right-4 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#E1306C] text-white shadow-lg transition-transform hover:scale-105 active:scale-95"
      >
        <Plus className="h-6 w-6" /><span className="sr-only">Add post for selected date</span>
      </button>
    </div>
  )
}
